...
class BYOL(nn.Module):
    def __init__(self, base_encoder, hidden_dim=4096, projection_dim=256, num_classes=15, moving_average_decay=0.99):
        ...
        #online
        self.online_encoder = nn.Sequential( #MLP
            self.base_encoder,
...
byol_transforms = T.Compose([
    ...
    T.ToTensor(), #Converts the image to a format suitable for PyTorch operations
...
criterion = byol_loss
classification_criterion = nn.BCELoss() #lossfun
...
for epoch in range(num_epochs):
    ...
    for images, _ in tqdm(train_loader):
        ...
        online_proj_one, online_proj_two, target_proj_one, target_proj_two = byol_model(images, images_transformed) #return the transformation
        loss = criterion(online_proj_one, online_proj_two, target_proj_one, target_proj_two) #return the loss
